...
export async function GET() {
  try {
    ...
    if (authError || !user) {
      ...
      return NextResponse.json<ErrorResponse>({ error: "Invalid or expired token" }, { status: 401 });
    }
    ...
    if (!profile) {
      ...
      return NextResponse.json<ErrorResponse>({ error: "Profile not found" }, { status: 404 });
    }
