...
async function fetchBriefs(params: ParsedBriefQueryParams): Promise<PaginatedResponse<BriefListItemDto>> {
  ...
  const response = await fetch(`${process.env.NEXT_PUBLIC_APP_URL}/api/briefs?${searchParams.toString()}`, {
...
async function fetchOwnedBriefCount(): Promise<number> {
  const response = await fetch(`${process.env.NEXT_PUBLIC_APP_URL}/api/briefs?filter=owned&limit=1`, {
    cache: "no-store",
  });
